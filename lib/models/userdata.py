...
def users(userid ,df, user_df, pool_df, city, cat, price, time, indicator):
    ...
    def like_price_city_cat_time(user_df,userid, city, cat, price, time):
    ...
    def dislike_price_city_cat_time(user_df,userid, city, cat, price, time):
        time = datetime.datetime.strptime(time, '%Y-%m-%d %H:%M:%S')
        user_df.loc[(user_df['UserId']==userid), city] -= 1
        user_df.loc[(user_df['UserId']==userid), cat] -= 1
        if time.hour >= 4 and time.hour < 7:
    ...
    if indicator == 0:
        user_data = dislike_price_city_cat_time(user_df,userid, city, cat, price, time)
    elif indicator == 1:
        user_data = like_price_city_cat_time(user_df,userid, city, cat, price, time)
