...
#userid = 0
import pandas as pd
def users(userid ,df, user_df, pool_df, city, cat, price,price_airbnb,beds_airbnb,people_airbnb,reviews_airbnb, time, indicator):
    #global userid 
    #print(df, user_df, pool_df, city, cat, price, time, indicator)
    #user_data = user_df
    #print('/////////////////////',indicator)

    def like(userid, city, cat, price,price_airbnb,beds_airbnb,people_airbnb,reviews_airbnb, time, user_df):
    ...
    def dislike(userid, city, cat, price,price_airbnb,beds_airbnb,people_airbnb,reviews_airbnb, time, user_df):
        time = datetime.datetime.strptime(time, '%Y-%m-%d %H:%M:%S')
        user_df.loc[user_df['UserId'] == userid, city] -= 1
        user_df.loc[user_df['UserId'] == userid, cat] -= 1
        if 4 <= time.hour < 7:
    ...
    data_temp = None
    if indicator == 0:
        data_temp = dislike(userid, city, cat, price,price_airbnb,beds_airbnb,people_airbnb,reviews_airbnb, time, user_df)
    elif indicator == 1:
        data_temp = like(userid, city, cat, price,price_airbnb,beds_airbnb,people_airbnb,reviews_airbnb, time, user_df)
    #print(user_data)
